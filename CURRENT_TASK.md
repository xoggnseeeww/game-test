...
- 위 3건 모두 `npm test`에 되돌리기 방지 검사를 넣고, **버그를 일부러 되살려 빨간불을 확인**했다.
...
## 다음 작업 우선순위
1. **ADHD 임계선 60이 리커트 한 칸 위에 걸려 있다** (채점 분석에서 나온 미해결 3건. 관련 파일:
   `js/tests/adhd/data.js` `AXIS_HIGH_THRESHOLD`·`QUESTIONS`, `js/tests/adhd/screens.js` 결과 화면)
   - 축당 4문항이라 원점수 한 칸 = 6.25%p. `raw 9 = 56%(낮음)` / `raw 10 = 63%(높음)` — 문항 하나에서
     "보통"↔"그렇다"가 유형 이름을 통째로 바꾸는데, DISC에 있는 `flat`/`nearTie` 같은 경고가 ADHD엔 없다.
   - **묵종 편향**: 전 문항 "그렇다"(3) → 각 축 63% → 임계선 초과 → **종합 태풍형**. 역채점 1문항으로는
     75%를 63%로 낮출 뿐 임계선을 못 넘긴다. 가장 흔한 무성의 응답이 가장 심각한 유형을 낸다.
     → 축당 역채점 2문항으로 늘리거나 임계선을 65로.
   - 임계선을 올리면 D-25의 "남은 문제"(보통이다로만 답한 사람이 게임만으로 임계선을 넘음)도 같이 닫힌다.
   - **문구 강도 불일치**: 63%일 때 `axisIntensityText()`는 "약간 높게"라는데 같은 화면의 유형 desc(`111`)는
     "뚜렷한 특징이 나타났어요"라고 한다.
2. **DISC 조합형 8개가 거의 안 나온다** — `SECONDARY_MIN=2`/`SECONDARY_GAP=3`이 빡빡해서 조합형 산출이
   14~15%. 잘 쓴 유형 본문 12개 중 8개가 사실상 사장되고 실질 유형이 4개다. 관련 파일: `js/tests/disc/score.js`
   ※ `SECONDARY_MIN=2`는 주석이 "1 SD"라고 하는데 실제 1 SD는 √(12/2)≈2.45다. 근거와 상수가 어긋나 있다.
3. **미니게임 목록이 비어 있음** — `/game`이 "준비 중" 빈 화면인데, 딜레마 게임은 이미 동작한다.
