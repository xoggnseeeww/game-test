...
## 현재 작업
없음. 2026-07-30: 목록 화면(심리테스트·미니게임)에 배너 추가 + 홈 하단 배너를 모바일
전용 고정(sticky) 도크로 전환(`.home-ad-dock`, 768px 미만에서만 `position:fixed`) —
"목록 화면엔 광고 없다"는 이전 판단을 사용자가 명시적으로 뒤집었고, "폰 하단에 붙는다고
생각하고 만들어달라"는 요청 반영. 데스크톱 카드 레이아웃(D-21)에선 고정하면 카드 폭을
무시하므로 일반 흐름 유지. 관련 결정: `docs/decisions/2027-h1.md` D-34.
`npm test` 66/66, `scripts/verify.cjs` 70/70. Playwright로 모바일 스크롤 전/후 위치 동일,
데스크톱 `position:static` 확인.

이전(2026-07-30, main 머지 완료): 배너가 상단+하단 2개인 화면에서 한쪽만 채워진다는 리포트 →
