...
## 현재 작업
없음 (게임 종료 → 결과 사이 광고 게이트 화면 추가 + 딜레마 게임 4택 재설계·데이터팬트리 연동을 메인과 머지, 2026-07-28)
...
- **광고 슬롯(배너·rect)** — 로더 주입까지 고쳤고 "슬롯마다 `<script>`가 붙는가"는 `scripts/verify.cjs`가 확인한다.
...
- **광고 게이트(interstitial, 300×250)** — 위와 같은 이유로 실제 노출은 미확인. `scripts/verify.cjs`가 카운트다운
  종료 후 버튼 활성화·게이트 경유는 확인하지만, 광고 자체가 그려지는지는 배포 후 실기기 확인 필요.
