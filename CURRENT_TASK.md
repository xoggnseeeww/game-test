...
- **ADHD 유형 desc 강도 문구 정리** — `RESULT_TYPES` 8개 중 6개에서 "뚜렷한"·"자꾸"·"유독"·"자주"·"잦은"·
  "늘"·"강해요"·"낮게 나왔어요" 같은 강도 단정 표현을 지우거나 중립화했다. `111`의 "뚜렷한 특징"이
  `axisIntensityText()`가 75%+ 전용으로 예약한 단어를 60%대(약간 높게)에서도 썼던 게 발단.
  `000`의 "낮게 나왔어요"도 대칭 문제였다(0~59% 전 구간을 아우르는데 59%는 "평범한 수준"). → D-30
- 위 7건 모두 `npm test`에 되돌리기 방지 검사를 넣고, **버그를 일부러 되살려 빨간불을 확인**했다.
