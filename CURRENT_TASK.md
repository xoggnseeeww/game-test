...
좌측 정렬이던 이미지 레이아웃을 세로 스택 + 수평 중앙 정렬로 고쳤다. 그 직후 "너무 기괴하다,
밝게 해달라"는 피드백을 받아, 어두운 카드 배경(`#0B0B0D`) + 글로우를 홈 화면 전역
`og-image.png`와 같은 원리(밝은 브랜드 색 → 진한 브랜드 색 그라디언트)로 재배색했다.
배포 후에도 "메인페이지로 간다"는 리포트가 계속돼 카카오 OG 캐시 도구로 재확인했더니
제목은 최신인데 이미지만 옛 버전 — `og-*.png`를 세 번 고치면서 매번 같은 파일명을 덮어써서
카카오의 이미지 캐시(HTML 스크랩 캐시와 별개 계층)가 안 갱신되고 있었다. `og-adhd-v2.png`
등으로 개명해 URL 자체를 새로 만들었다. **그래도 안 풀려서** 사용자에게 브라우저 개발자
도구로 직접 확인을 부탁했고, `308 Permanent Redirect` → `location: /og-shells/test-disc`
(확장자 없음)를 발견 — **진짜 원인**은 Cloudflare Pages가 `.html` 파일을 확장자 없는
주소로 정규화하면서, `_redirects`가 `.html`이 붙은 목적지로 rewrite하니 308을 한 번 더
쐈고 그 목적지가 앱에 없는 경로라 라우터가 홈으로 떨어뜨린 것. `_redirects` 목적지에서
`.html`을 뺐다(`/og-shells/test-disc` 등). `serve.py`도 이 clean-URL 동작을 흉내 내도록
고쳐서 로컬에서 재현·검증 가능해졌다. 관련 결정: `docs/decisions/2027-h1.md`
D-32(2026-07-30 갱신 4건). 아직 main 머지 전.
**배포 후 실제로 308이 사라졌는지, 카카오 디벨로퍼스 OG 캐시 삭제(세 URL)를 다시 해야 함**
— 아래 "배포 후 확인 필요" 참고.
...
- **⚠️ `_redirects`의 `.html` 확장자 제거가 실제로 308을 없앴는지** — 로컬은 `serve.py`가
  Cloudflare의 clean-URL 동작(확장자 없는 경로 → 같은 이름 `.html` 파일)을 흉내 낸 것으로만
  검증했다. 배포본에서 개발자 도구 Network 탭을 열고 `/test/adhd`·`/test/disc`·
  `/game/numpath`에 직접 접속해서 **첫 요청의 Status Code가 200인지(308이 다시 뜨면 실패)**,
  주소창이 그대로 유지되는지, `view-source:`로 `<head>`가 정적 셸 내용인지, 이어서 SPA가
  정상 렌더되는지 확인 필요. 크롤러 시점 확인은 카카오톡 공유 미리보기 또는
