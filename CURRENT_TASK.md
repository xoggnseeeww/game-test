...
차단으로 연결하지 않은 게 원인 — 새 기능이 아니라 장식이던 필드를 실제로 켜는 수정.
`attemptMove()`에 `isOutOfMoves()` 가드 추가 + `renderBoard()`가 제한 도달 시
`np-board--locked`(클리어와 같은 클래스, `pointer-events:none`) 적용. `engine.js`의
`canEnter()`는 그대로 뒀다 — 거기서 moveLimit까지 보면 `isStuck()`/`isOutOfMoves()`가 항상
같이 true가 돼 기존 "서로 배타적" 계약(`test/numpath.engine.test.js`)이 깨진다.
실제 브라우저(임시 playwright 스크립트, 레포엔 없음)로 "target을 안 지나고 moveLimit을
정확히 채우는 경로"를 찾아 클릭 → 제한 도달 후 board 잠김·강제 클릭해도 이동 횟수 불변까지
확인, 수정 전 코드로 같은 스크립트가 빨간불 뜨는 것도 확인(회귀 재현).
**부수 수정**: 확인 과정에서 `scripts/verify.cjs`의 기존 Undo/Reset 검사가 낮은 확률(약 5%)로
멈추는 경쟁 조건을 발견 — `solveNumpath()`가 찾은 첫 해가 우연히 1수짜리면 그 수를 클릭하는
순간 클리어돼 Undo가 비활성 상태로 굳었다. 이번 수정과 무관한 기존 버그(수정 전 코드로도
재현)라 `firstNonClearingMove()` 헬퍼로 같이 고쳤다.
`npm test` 135/135(불변), `scripts/verify.cjs` `serve.py`로 5연속 통과 확인.
...
## 다음 작업 우선순위
- **`docs/decisions/2027-h1.md` 파일 크기 분리 필요** — 649줄/약 68KB로 자동 분리 규칙
  (300줄/15KB)을 넘겼다(2026-07-30, 4차 머지 확인). 대부분(D-32~D-53)이 부부 체크 병렬 세션이
  쌓은 내용이라 이번 세션에서 임의로 재구성하면 그쪽 맥락을 잘못 옮길 위험이 있어 보류했다.
  다음에 이 파일을 만지는 세션이 규칙대로 `docs/decisions/2027-h2.md`를 새로 만들어 향후
  항목부터 받게 하고(과거 항목은 그대로 둔다 — 2026-h2.md → 2027-h1.md 분리 때와 같은 방식),
  `docs/DECISIONS.md` 헤더에 "3차 분리" 안내를 추가할 것.
