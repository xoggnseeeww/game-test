...
미리보기 추가(main 머지 완료) — `og-shells/*.html` + `_redirects` rewrite 규칙(와일드카드 위) +
앱별 `assets/og-*.png`(1200×630). `test/og-shells.test.js` 신규(카드 데이터와 셸 내용 대조).
배포 후 사용자가 카카오톡 채팅 미리보기 스크린샷으로 "아이콘+제목이 잘려 보인다"고 지적 —
카카오톡 채팅 썸네일이 `og:image`를 가운데 정사각형(630×630)으로 크롭한다는 걸 확인하고,
좌측 정렬이던 이미지 레이아웃을 세로 스택 + 수평 중앙 정렬로 다시 만들었다(파일명 불변이라
셸·테스트는 안 건드림). 관련 결정: `docs/decisions/2027-h1.md` D-32(2026-07-30 갱신 포함).
