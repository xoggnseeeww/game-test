...
## 현재 작업
브랜치 `claude/numpath-stack-clear-design-yk5lyt`에서 작업 중, **아직 main 미병합**.
2026-07-28: 독립 미니게임 **NumPath: Stack & Clear**(숫자 경로 퍼즐) 추가.
...
`npm test` 56/56, `scripts/verify.cjs` 64/64(로컬, 여러 차례 재현) 통과. 아래 "다음 작업
우선순위" 1번(미니게임 목록 비어 있음)이 이걸로 해소됨 — `renderGameList()`가 이제
`listGames()`로 카드를 자동 생성한다.
...
데이터팬트리 연동. 상세는 `PROGRESS.md` 2026-07-28 항목, 관련 결정은 `docs/DECISIONS.md`
D-24~D-27.

## 다음 작업 우선순위
1. **NumPath 브랜치를 main에 병합** — 지금은 로컬 브랜치에만 있다. 리뷰·PR 필요 여부 확인 후 진행.
2. ~~미니게임 목록이 비어 있음~~ — NumPath 추가로 해소됨(위 참고).
