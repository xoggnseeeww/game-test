...
## 현재 작업
없음 (광고 슬롯 점선 테두리 제거 + AdFit SPA 재스캔 버그 수정, 메인 머지 완료, 2026-07-28)

- **광고 슬롯 점선 테두리 제거 + AdFit SPA 재스캔 버그 수정**: `.ad-slot`의 `border: 1.5px dashed`를 제거(요청).
  이어서 "광고가 뜨다가 말다가 한다"는 리포트 확인 — `ba.min.js`는 로드 시점의 DOM만 스캔하는데, SPA라
  화면 전환마다 `router.js`의 `render()`가 `app.innerHTML`을 지우고 새로 그리다 보니 최초 로드 이후 생기는
  광고 슬롯은 재스캔되지 않아 비어 있었다. `js/core/ads.js`에 `refreshAds()`(스크립트 태그를 새로 갈아끼워
  재실행)를 추가하고, `router.js`의 `render()`에서 새로 그린 화면에 `.kakao_ad_area`가 있으면 호출하도록 연결.
  `npm test` 31/31, `scripts/verify.cjs` 28/29(외부 CDN 403은 샌드박스 프록시 제약, 변경 전에도 있던 것 — 실제
  광고 노출 재확인은 여전히 배포 후 실기기에서 필요).
...
- **광고 슬롯** — AdFit 코드 연동 + SPA 재스캔 버그 수정(2026-07-28) 완료. 샌드박스에서 외부 스크립트(`t1.daumcdn.net`)가
  막혀 실제 노출/렌더링은 여전히 배포 후 실기기로 확인 필요 — 특히 홈→테스트 화면 등 여러 번 이동하며 매번 광고가 뜨는지
