...
// 목록 카드는 등록된 학습 유닛에서 뽑아 쓴다(renderGameList와 같은 패턴).
export function renderLearningList() {
  const units = listLearning();
  app.appendChild(el(`
    <div>
      <div class="back-row">
        <button class="back-btn" data-nav="home">‹</button>
      ...
      <div class="test-list">
        ${units.map((u) => `
          <button class="test-card" data-nav="${u.card.screen}">
            <div class="icon" style="background:${u.card.color};">${u.card.emoji}</div>
            <div class="body">
              <div class="name">${u.card.name}</div>
              <div class="desc">${u.card.desc}</div>
            </div>
            <div class="chevron">›</div>
          </button>
        `).join("")}
