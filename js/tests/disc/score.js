...
// 2위가 이 값 미만이면 조합형으로 보지 않는다. 무작위 응답의 축별 SD가 약 2.45라
// 2는 0.8 SD쯤이고, "1 SD 미만은 노이즈"라던 예전 주석은 상수와 어긋나 있었다(2 < 2.45).
// 실제 역할은 **평평한 사람을 걸러내는 최저선**이다 — 3으로 올리면 조합형이 12.5%까지
// 떨어져 유형 본문 8개가 다시 묻힌다.
const SECONDARY_MIN = 2;

// 1·2위 차이가 이보다 크면 조합형이 아니라 순수형. 3에서 5로 완화했다.
// 3일 때는 진짜로 두 축이 비슷한 응답자를 절반(50.9%)밖에 조합형으로 못 잡아,
// 공들여 쓴 유형 본문 12개 중 8개가 각각 2% 남짓에 머물렀다. 5에서 재현율 63.8%,
// 순수형을 조합형이라 부르는 오탐은 1.7% → 3.6%. → `docs/DECISIONS.md` D-29
