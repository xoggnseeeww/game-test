...
- **해결**: `js/core/ads.js`의 `mountAds(root)`가 화면을 그린 뒤(`router.js`의 `render()` 끝) **아직 로더가 붙지 않은 슬롯마다
  로더 `<script>`를 DOM API로 만들어 `<ins>` 바로 뒤에 붙인다.** AdFit은 로더 직전의 `<ins>`를 자기 슬롯으로 본다.
- **함정**: `innerHTML`로 넣은 `<script>`는 브라우저가 **실행하지 않는다.** 그래서 `adSlotMarkup()` 문자열에 로더를 끼워 넣는 방식은 통하지 않는다.
- **일반 원칙**: **"페이지 로드 시 1회 스캔"으로 동작하는 서드파티 스크립트는 전부 이 SPA와 충돌한다.**
  새로 붙일 때는 "화면 전환 후에도 다시 붙는가"를 먼저 묻는다.
- **검증**: `scripts/verify.cjs`의 "광고 슬롯마다 AdFit 로더가 붙음" 2건이 막는다(홈 + SPA 이동 후).
