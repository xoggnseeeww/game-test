...
`basic-conversation`(기초 영어회화) 하나, 그 안 챕터는 8개다 — 인사/기분 표현(30문장),
아침 일과·가족 이야기·친구랑 놀기·밥 먹을 때·목욕/잠자리·날씨 이야기·자기소개(각 8~10문장).
단어 나열이 아니라 일상대화 위주로 구성했다(어린이 애니메이션 참고, D-65). 광고 슬롯은
아직 없다(파일럿 단계라 뺐다). 서버 API 없이 브라우저 내장
