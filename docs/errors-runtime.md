# errors-runtime — 런타임 · 브라우저 · 배포 오류

> 선택 로드 문서. `docs/ERRORS.md`의 **인덱스에서 E-번호를 찾은 뒤** 이 파일을 읽는다.
> `ERRORS.md`가 자기 분리 규칙(300줄)을 넘겨(360줄) 2026-08-19에 나눴다(D-105) —
> 규칙에 적혀 있던 분리 대상 파일명(`errors-runtime.md`/`errors-ai.md`)을 그대로 썼다.
> AI가 반복하는 실수 패턴(A-번호)은 `docs/errors-ai.md`에 있다.
...
### E-15. `oauth-code-exchange-never-fires` — PKCE로 바꿔도 여전히 로그인이 안 될 수 있다
E-14(PKCE 전환)를 배포한 뒤에도 로그인이 안 되는 사례가 있었다(D-77). `get_logs(auth)`엔 `/authorize`→`/callback`
성공이 찍혀 있는데, 그 직후 코드를 실제 토큰으로 바꾸는 GoTrue `/token` 요청 자체가 로그에 없다 — 브라우저가
코드 교환을 **시도조차 안 했다**는 뜻.

- **원인**: `detectSessionInUrl`(기본값 `true`)의 "URL의 `code`를 자동 감지해 교환한다"는 동작을 그냥 믿고
  있었다. 이 앱은 `@supabase/ssr`의 `createBrowserClient`(쿠키 기반, PKCE 자동 처리가 잘 다져진 경로)가
  아니라 순수 `createClient`를 **동적 import로 늦게** 띄우는 구성이라(`cloud-auth.js` 파일 헤더), 이 조합에서
  자동 감지가 실제로 매번 발동하는지 실기기로 확증할 방법이 없었다(D-55 한계로 진짜 OAuth 왕복 자체를
  재현 못 함).
- **해결**: `detectSessionInUrl: false`로 끄고, 부팅 시 `location.search`의 `code`를 직접 읽어
  `exchangeCodeForSession(code)`를 명시적으로 호출한다. 실패하면 `console.error` + `getAuthError()`로
  노출하고, `renderSignInButton()`이 그 값을 읽어 버튼 대신 실패 사유를 보여준다.
- **일반 원칙**: "라이브러리가 알아서 처리해준다"는 자동 동작은, 그게 실패했을 때 **아무 신호도 안 남기는
  구성**(에러 없음·재시도 없음·로그 없음)과 만나면 몇 번을 고쳐도 원인을 못 좁힌다 — 이 사건에서 실제
  버그는 한 번(D-77)뿐이었어도, 그 전 두 시도(D-75·D-76)가 전부 "확인할 방법이 없어서" 진짜 원인인지
  아닌지도 모른 채 넘어갔다. 자동 동작에 기대는 코드에는 **그게 실패했을 때 보이는 신호**를 반드시
  같이 심는다.
- **검증**: `npm test` 141/141, `scripts/verify.cjs` 전부 통과. 배포 후 사용자가 실기기에서 재확인 —
  **정상 동작**.
