...
전부 나열하지 말고 **끝에 결과 요약만** 남긴다. "꼭 필요한 방향 전환만 한 줄로 알리고"조차
반복 지적으로 뺐다(2026-08-01 → 2026-08-06) — "이제 ~할게요", "~인지 확인해보자" 같은 전환
문장 하나짜리도 도구 호출 사이사이에 계속 나오면 그 자체가 나열이다. 도구 호출 전 한 줄
안내(하네스가 요구하는 최소 상태 고지)까지는 막을 수 없지만, 그 뒤에 "왜 이 결과가 나왔는지",
"다음엔 뭘 확인할지"를 또 설명하는 게 반복되는 패턴이었다 — 막힌 것이 아니라 실제로 두 번 더
재지적됐다(사용자가 md 파일에 아예 박아두라고 요청). 진짜 막힌 것/디버깅 중 정말 중요한 발견
(예: 예상과 다른 원인 발견, 계획 변경)만 예외로 한 줄 남기고, 그마저 애매하면 생략 쪽으로
기운다.
...
js/learning/<toolId>/ 학습 카테고리 안의 독립 도구 1개 = 폴더 1개(게임과 같은 레지스트리
                      ...
                      챕터 중 붙일 자리가 있는지 먼저 본다. 도구 둘: **basic-conversation**
                      ...
                      않는다 — 지금은 저학년 챕터 5개(학교 가는 날·교실에서·쉬는 시간·
                      좋아하는 것·체육 시간, 챕터당 20문장, D-80·D-83), 중학년 챕터
                      5개(수업과 과제·친구 관계·방과 후 생활·우리 동네·학교 행사,
                      챕터당 20문장, D-81·D-83), 고학년 챕터 5개(의견 나누기·꿈과 진로·
                      요즘 이슈·여행과 경험·협동과 리더십, 챕터당 20문장, D-82·D-83)까지
                      있다 — 3개 학년·15챕터·300문장. 학년마다 문법 목록
                      (`*_GRAMMAR_POINTS`)이 따로 있어 서로 독립적이고, 개념이 겹쳐도
                      (예: 저학년 G2와 중학년 G7이 둘 다 일반동사 현재형) 각자 새 id로
                      다시 정의한다 — 새 문법 id 번호는 그 학년 안에서가 아니라 지금까지
                      나온 것 중 가장 큰 다음 번호를 전체에서 이어 붙인다(D-83). 문법은
                      챕터마다 무조건 새로 늘리지 않는다 — 새 문법 없이 기존 걸 다른
                      소재로 재사용만 하는 챕터도
                      있다(좋아하는 것, 우리 동네, 여행과 경험, 체육 시간). 진짜 다중 턴
                      대화형 학습은 두 도구 다 아직 없다 — 이 앱의 "문장 하나 듣고 따라
                      말하기" 구조와 안 맞아 별도 도구로 나중에 다룰 계획. 한글을 아직 못
                      읽는 어린이를 위해 한국어 해석도 TTS로 들을 수 있다(뜻 옆 🔊, D-69) —
                      영어/한국어 듣기가 서로 다른 `lang`으로 독립적으로 재생된다. 브라우저
                      내장 TTS/STT만 쓰고 서버 API 없음(로그인·학습 진행률 동기화는 D-68
