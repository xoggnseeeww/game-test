...
js/learning/<toolId>/ 학습 카테고리 안의 독립 도구 1개 = 폴더 1개(게임과 같은 레지스트리
                      ...
                      영어회화, 7세 이하 대상) 하나, 그 안 챕터는 인사/기분 표현(30문장)·
                      하루 일과(아침+밥+목욕/잠자리 통합, 32문장)·가족·자기소개(24문장)·
                      놀이터에서(놀이+날씨 통합, 26문장) 4개, 총 112문장 — 일상대화 위주,
                      어린이 애니메이션 참고(D-65). 같은 문장 틀에 단어만 바꿔 반복하는
                      패턴 드릴("I like ~", "This is my ~", "I want ~", "Let's play ~")도
                      섞여 있다(D-67). 한글을 아직 못 읽는 어린이를 위해 한국어 해석도
                      TTS로 들을 수 있다(뜻 옆 🔊, D-68) — 영어/한국어 듣기가 서로 다른
                      `lang`으로 독립적으로 재생된다.
                      브라우저 내장 TTS/STT만 쓰고 서버 API·로그인·영속 저장 없음. 듣기/
                      말하기를 강제하지 않고 건너뛰기 버튼도 둔다(D-61). 문장 카드 위 마스코트
