...
test("_redirects: OG 셸 규칙 3개가 전부 존재하고 대상 파일도 실재한다", () => {
  ...
  for (const shell of SHELLS) {
    ...
    assert.equal(rule.dest, shell.dest, `${shell.path} 규칙의 대상 파일이 예상과 다르다`);
    assert.equal(rule.status, "200", `${shell.path} 규칙은 rewrite(200)여야 한다 — 리다이렉트면 주소창이 바뀐다`);
    assert.ok(
      fs.existsSync(path.join(ROOT, shell.dest)),
      `${shell.dest} 파일이 실제로 없다`
    );
  }
});
...
test("OG 셸: title·og:title·og:description이 카드 데이터와 일치한다", () => {
  for (const shell of SHELLS) {
    const html = fs.readFileSync(path.join(ROOT, shell.dest), "utf8");
    const expectedTitle = `${shell.card.name} | 과몰입구역`;

    const titleMatch = html.match(/<title>([^<]*)<\/title>/);
    assert.ok(titleMatch, `${shell.dest}에 <title>이 없다`);
    assert.equal(titleMatch[1], expectedTitle, `${shell.dest}의 <title>이 카드 이름과 다르다`);

    const ogTitleMatch = html.match(/og:title" content="([^"]*)"/);
    assert.ok(ogTitleMatch, `${shell.dest}에 og:title이 없다`);
    assert.equal(ogTitleMatch[1], expectedTitle, `${shell.dest}의 og:title이 카드 이름과 다르다`);

    const ogDescMatch = html.match(/og:description" content="([^"]*)"/);
    assert.ok(ogDescMatch, `${shell.dest}에 og:description이 없다`);
    // card.desc는 " · "로 태그를 잇는 문구라 셸 문구엔 그 앞부분만 그대로 들어간다.
    const descPrefix = shell.card.desc.split(" · ")[0];
    assert.ok(
      ogDescMatch[1].includes(descPrefix),
      `${shell.dest}의 og:description이 카드 설명("${descPrefix}")을 반영하지 못했다 — 문항 수 등이 바뀌었으면 셸도 같이 갱신할 것`
    );
  }
});
...
test("OG 셸: og:image가 실제 존재하는 파일을 가리킨다", () => {
  for (const shell of SHELLS) {
    ...
    assert.ok(imgMatch, `${shell.dest}에 og:image가 없다`);
    assert.equal(imgMatch[1], shell.image, `${shell.dest}의 og:image 파일명이 예상과 다르다`);
    assert.ok(
...
test("OG 셸: og:url이 실제 경로와 정확히 일치한다(트레일링 슬래시 불일치 없음)", () => {
  for (const shell of SHELLS) {
    ...
    assert.ok(urlMatch, `${shell.dest}에 og:url이 없다`);
